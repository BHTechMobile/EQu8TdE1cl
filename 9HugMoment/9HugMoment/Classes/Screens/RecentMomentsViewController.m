...
@implementation RecentMomentsViewController{
    RecentsMomentsModel *recentsMomentsModel;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    recentsMomentsModel = [[RecentsMomentsModel alloc] init];
...
- (void)setupTable{
    for (int i=0; i<recentsMomentsModel.messages.count; i++) {
...
- (void)getAllMessage {
    ...
    [_hud show:YES];
    [recentsMomentsModel getAllMessagesSuccess:^(id result) {
        NSLog(@"count : %lu",(unsigned long)recentsMomentsModel.messages.count);
...
- (void)setupTableView:(NSIndexPath *)indexPath{
    ...
    _arrayDataSource = [[ArrayDataSource alloc]initWithItems:recentsMomentsModel.messages
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    message = [recentsMomentsModel.messages objectAtIndex:indexPath.row];
...
- (void)refreshTableView {
    [recentsMomentsModel getAllMessagesSuccess:^(id result) {
        NSLog(@"Message found: %lu",(unsigned long)recentsMomentsModel.messages.count);
